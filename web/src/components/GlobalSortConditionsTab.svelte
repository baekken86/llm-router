<script>
  ...
  import { addToast, sortConditions } from '../lib/stores.js';
  ...
  const previewSortExpr = $derived.by(() => {
    if (editingId !== null) {
      return Array.isArray(formSortExpr) ? formSortExpr : [];
    }
    return sorted
      .filter(c => c.enabled !== false)
      .flatMap(c => (Array.isArray(c.sort_expr) ? c.sort_expr : []));
  });
  ...
  async function load() {
    loading = true;
    try {
      conditions = await apiFetch('/api/v1/global-sort-conditions');
      sortConditions.set(conditions);
    } catch (e) {
  ...
  async function toggleEnabled(c, checked) {
    ...
    try {
      await apiFetch(`/api/v1/global-sort-conditions/${c.id}`, {
        method: 'PUT',
        body: {
          name: c.name,
  ...
  async function save() {
    ...
    try {
      if (editingId === 'new') {
        await apiFetch('/api/v1/global-sort-conditions', { method: 'POST', body });
        addToast('Created', 'success');
      } else {
        await apiFetch(`/api/v1/global-sort-conditions/${editingId}`, { method: 'PUT', body });
  ...
  async function moveRow(fromIdx, dir) {
    ...
    // enforce full order list (arrays must contain every id)
    conditions = next;
    try {
      conditions = await apiFetch('/api/v1/global-sort-conditions/reorder', {
        method: 'PUT',
  ...
  async function deleteCondition(id) {
    try {
      await apiFetch(`/api/v1/global-sort-conditions/${id}`, { method: 'DELETE' });
...
<div>
  <div class="flex items-center justify-between mb-6">
    <h2 class="text-xl font-bold text-gray-100">Sort Conditions</h2>
    <button
      ...
      <form onsubmit={(e) => { e.preventDefault(); save(); }} class="space-y-4 max-w-3xl">
        <div>
          ...
          <input
            type="text"
            bind:value={formName}
            placeholder="e.g. prioritize-cheap, docs-first"
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="What does this sort condition do?"
            rows="2"
            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-emerald-500"
          ></textarea>
        </div>
  ...
  {:else if filtered.length === 0}
    <p class="text-gray-500">No sort conditions defined.</p>
  {:else}
        ...
        <tbody>
          {#each filtered as c, i (c.id)}
            <tr class="border-b border-gray-850 hover:bg-gray-850/50">
              ...
              <td class="px-4 py-2.5 text-gray-400 text-xs font-mono max-w-md truncate" title={sortSummary(c.sort_expr)}>
                {sortSummary(c.sort_expr)}
              </td>
  ...
  {#if previewOpen}
    <div class="mt-8 bg-gray-900 border border-gray-800 rounded-lg p-4">
      <h3 class="text-sm font-medium text-gray-300 mb-2">
        Preview <span class="text-gray-500 text-xs">(enabled conditions</span>
        <span class="text-gray-500 text-xs">— per-VM overrides may disable some of these)</span>
      </h3>
      <ResolvedPreview
        previewMode={true}
        composition={{ filter_expr: null, sort_expr: previewSortExpr }}
      />
    </div>
  {/if}
</div>
